% Imports AIS data and sets up ship vectors to move 
...
idn = '/Users/julievanderhoop/Documents/MATLAB/UGthesis/';
...
% from domain, lat of exit from domain, length (m), width (m), heading 
% (degrees), delta northing (nmi), delta easting (nmi), delta northing 
% (degrees), delta easting (degrees), greater circle distance of track (m),
% datenum of entry into domain, zeros, distance/step, distance/step east-
% west, distance/step north-south, datenum of exit from domain]
for i = 1:length(sepvess)
...
for i=1:size(ship,1);
    time(i,7)=ship(i,1);
    time(i,8)=ship(i,2);
    time(i,12)=etime(datevec(ship(i,20)),datevec(ship(i,15))); % time elapsed in domain
    time(i,13)=time(i,12)/3.5; % number of steps in domain
end
...
for i=1:size(ship,1);
    ...
    time(i,8)=ship(i,2);
    time(i,12)=etime(datevec(ship(i,20)),datevec(ship(i,15))); % time elapsed in domain
    time(i,13)=time(i,12)/3.5; % steps in domain
    ship(i,21)=time(i,13);
end
time=sortrows(time,12);
ii=find(ship(:,21)>=0);
